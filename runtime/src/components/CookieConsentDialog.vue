...
function onClosed() {
  if (_resolve) { _resolve({ ...consent.value }); _resolve = null }
  if (_saved) {
    setTimeout(() => {
      Modal.confirm({
        title: '需要重新加载页面',
  ...
  <DialogView v-model="open" @closed="onClosed">
    ...
    <ul class="categories">
      <li v-for="cat in CONSENT_CATEGORIES" :key="cat.key" class="category">
        ...
        <label class="toggle">
          <input type="checkbox" :checked="consent[cat.key]"
            :disabled="cat.required || (!!consent.ns && (cat.key === 'p' || cat.key === 't'))"
...
<style scoped>
.dialog-desc { font-size: 14px; color: #666; margin: 0 0 20px; line-height: 1.5; }
.categories { list-style: none; margin: 0; padding: 0; }
.category { display: flex; align-items: center; gap: 12px; padding: 12px 0; border-bottom: 1px solid #eee; }
.category:last-child { border-bottom: none; }
.category-body { flex: 1; min-width: 0; }
.category-label { font-size: 14px; font-weight: 600; }
.category-desc { font-size: 12px; color: #888; margin-top: 2px; }
.toggle { flex-shrink: 0; position: relative; width: 44px; height: 24px; display: block; }
.toggle input { position: absolute; opacity: 0; width: 100%; height: 100%; margin: 0; cursor: pointer; }
.toggle input:disabled { cursor: not-allowed; }
.toggle-track { display: block; width: 100%; height: 100%; border-radius: 12px; background: #ccc; transition: background 0.2s; }
.toggle input:checked + .toggle-track { background: #4a90d9; }
.toggle input:disabled + .toggle-track { opacity: 0.6; }
.toggle-track::after { content: ''; position: absolute; top: 2px; left: 2px; width: 20px; height: 20px;
...
.btn { flex: 1; padding: 10px 16px; border: none; border-radius: 8px; font-size: 14px; font-weight: 600; cursor: pointer; transition: opacity 0.15s; }
