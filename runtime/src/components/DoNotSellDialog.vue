...
function onClosed() {
  if (_resolve) { _resolve(_confirmed); _resolve = null }
  if (_confirmed) {
    setTimeout(() => {
      Modal.confirm({
        title: '需要重新加载页面',
...
<style scoped>
.dns-desc { font-size: 14px; color: #666; margin: 0 0 20px; line-height: 1.5; }
.option { display: flex; align-items: flex-start; gap: 12px; padding: 14px 16px; border: 1px solid #ddd;
  border-radius: 8px; background: #fafafa; margin-bottom: 20px; }
.option input[type="checkbox"] { margin-top: 2px; flex-shrink: 0; width: 18px; height: 18px; cursor: pointer; }
.option-body { flex: 1; }
.option-label { font-size: 14px; font-weight: 600; }
.option-desc { font-size: 12px; color: #888; margin-top: 4px; line-height: 1.4; }
